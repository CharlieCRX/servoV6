...
| 绝对定位触发[i] | M48..M63 | 上升沿启动 | PLC 自动复位；客户端只写 ON，无需配对 OFF，但应读回确认 |
| 相对定位触发[i] | M64..M79 | 上升沿启动 | PLC 自动复位；客户端只写 ON，无需配对 OFF，但应读回确认 |
...
| 相对定位终止触发[i] | M144..M159 | 上升沿停止 | PLC 自动复位；客户端只写 ON，无需配对 OFF，但应读回确认 |
| 绝对定位终止触发[i] | M160..M175 | 上升沿停止 | PLC 自动复位；客户端只写 ON，无需配对 OFF，但应读回确认 |
...
> 更新（2026-08，PLC 当前版本）：触发/终止线圈 M48/M64/M144/M160 由 PLC 自动复位，
> 客户端只需写 ON 即可，不再需要产生 ON→OFF 脉冲，也无需在通信异常后主动清 OFF。
> "读回确认"由上位机 telemetry/ack reader 异步读取完成（命令 writer 只负责提交，
> `CommunicationResult::ok()` 仅证明写请求到达，不证明 PLC 已执行并自动复位）。

统一客户端规则：

- 保持电平：本机负责最终 OFF。
- PLC 自复位（触发/终止/清除）：只写 ON，等待 PLC 读回 OFF；若超时未复位，报告故障并主动清 OFF。
